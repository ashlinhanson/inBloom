...
function Navbar(props) {
  ...
  useEffect(() => {
    tl.from(
      ".homes",
      { x: 15, opacity: 0, ease: Power3.easeOut, delay: 0.1 },
      "Start"
    );
    tl.from(
      ".search",
      { x: 15, opacity: 0, ease: Power3.easeOut, delay: 0.2 },
      "Start"
    );
    tl.from(
      ".contact",
      { x: 15, opacity: 0, ease: Power3.easeOut, delay: 0.3 },
      "Start"
    );
  }, []);
  ...
  return (
    <nav className="navbar navbar-expand-lg navbar-light bg-light mt-n5">
      <a className="navbar-brand homes text-success" href="#">
        inBloom
      </a>
      <button
          ...
          <li className="nav-item active search">
            <form className="form-inline">
              <div className="form-group mx-sm-3 mb-2">
                <label htmlFor="plantSearch" className="sr-only">
                  Search :{" "}
                </label>
                <input
                  type="search"
